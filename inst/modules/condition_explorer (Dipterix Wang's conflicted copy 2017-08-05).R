...
SHINY_EXECUTE = function(params, use_cache = TRUE, ...){
  ...
  if(!is.null(params[['.IS_SUMA']])){
    ...
    if(baseline_ind[1] == 1) {
      tmp = as.array(data_env$cumsum[trial_ind,,baseline_ind[n_b], sel_electrodes])
      cbt_baseline <- tmp / n_b
      rm(tmp)
    } else {
      tmp1 = as.array(data_env$cumsum[trial_ind,, baseline_ind[n_b], sel_electrodes])
      tmp2 = as.array(data_env$cumsum[trial_ind,, baseline_ind[1]-1, sel_electrodes])
      cbt_baseline <- (tmp1 - tmp2) / n_b
      rm(tmp1, tmp2)
    }


    tmp = alloc_vector(dim(cbt_baseline))
    # cbt_baseline = as.array(cbt_baseline)

    # tmp = as.array(data_env$data[trial_sel,,, sel_electrodes])
  ...
  cbt_baseline = as.array(cbt_baseline)
  tmp1 = as.array(data_env$data[trial_sel,,, sel_electrodes])

  # tmp = as.vector(cbt_baseline)
  # content_bc = 100 * (content - tmp) / cbt_baseline

  content_bc = 100 * vapply(1:length(time), function(ii) {
    re = (tmp1[,,ii,] - cbt_baseline[,,,]) / cbt_baseline[,,,]
    as.array(re)
  },cbt_baseline)
