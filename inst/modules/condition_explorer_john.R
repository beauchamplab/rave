...
SHINY_EXECUTE = function(params, ...){
  ...
  content <- data_env$data[,,,params$selected_electrode == data_env$electrodes]

  # cumsum_by_trial <- repo$summary$cumsum_by_trial
  # if you have multiple files and want to source them,
  # use relative path to the root dir `rave`
  ...
  #   return (list(heatmap_plot=default_plot, line_plot=default_plot))
  # }
  ...
  frequencies <- data_env$subject$frequencies %>% round
  time <- data_env$subject$time %>% round(3)
  ...
  # make sure this to use the baseline corrected VALUES!!!!!
  if(!is.null(params[['.IS_SUMA']])){
    ...
    if(baseline_ind[1] == 1) {
      cbt_baseline <- cumsum_by_trial[trial.ind,,baseline_ind[n_b],1] / n_b
    } else {
      cbt_baseline <- (cumsum_by_trial[trial.ind,, baseline_ind[n_b],1] - cumsum_by_trial[trial.ind,, baseline_ind[1]-1,1]) / n_b
    }
  ...
  # logger(params$selected_electrode)
   cumsum_by_trial <- data_env$cumsum[,,,params$selected_electrode == data_env$electrodes]

   # logger(paste(dim(cumsum_by_trial), collapse = ' '))

  if(baseline_ind[1] == 1) {
    cbt_baseline <- cumsum_by_trial[trial_sel,,baseline_ind[n_b]] / n_b
  } else {
    cbt_baseline <- (cumsum_by_trial[trial_sel,, baseline_ind[n_b]] - cumsum_by_trial[trial_sel,, baseline_ind[1]-1]) / n_b
  }
  ...
  return(list(
    ...
    trial_plot = function () {
      ...
      if(is.null(dataB)) {
        do_img(trials_over_timeA, main=params$control_nameA)
      } else if (is.null(dataA)) {
        do_img(trials_over_timeB, main=params$control_nameB)
      } else {
        par(mfrow=1:2)
        do_img(trials_over_timeA, main=params$control_nameA)
        do_img(trials_over_timeB, main=params$control_nameB)
