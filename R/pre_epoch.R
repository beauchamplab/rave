  ...
  server = function(input, output, session, user_data, utils){
    ...
    observe({
      ...
      if(utils$notch_filtered()){
        ...
        if(length(epochs)){
          epochs = str_match(epochs, '^epoch_(.+).[cC][sS][vV]')[,2]
        }
    ...
    output$inner_ui2 <- renderUI({
      ...
      if(length(selected_files)){
        selected_files = str_replace(selected_files, sprintf('^%sDatafile[\\w]+_', subject_code), sprintf('%sDatafile%s_', subject_code, block))
    ...
    plot_volt = function(time, signal, plot_range, use_abs, vlines = NULL, hlines = NULL, vcols = 2, ...){
      ...
      plot(time, signal, type = 'l', ylim = ylim, main = main, las = 1, xlab = 'Time (s)', ylab = 'Strength', ...)
      if(!is.null(hlines)){
        abline(h = hlines, col = 'blue')
      }
      if(!is.null(vlines)){
        abline(v = vlines, col = vcols)
      }
    }
