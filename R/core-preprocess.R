...
rave_preprocess_tools <- function(env = new.env(), ...){
  ...
  local({
    ...
    check_load_subject <- function(subject_code, project_name, details = TRUE, strict = TRUE){
      checklist <- list()
      ind <- 1
    ...
    get_check_level <- function(){
      if(!utils$has_subject()){
        ...
        checklevel <- utils$get_from_subject('checklevel', 0, customized = TRUE)
      }
      return(checklevel)
    }
    ...
    get_from_subject <- function(key, default, customized = FALSE){
      if(customized){
        logger <- get_val(env$subject, 'logger', default = default, .invalids = 'null')
        if(inherits(logger, 'RAVEHistory')){
          logger$get_or_save(key, default, save = FALSE)
        }else{
          return(default)
        }
      }else{
    ...
    set_srate <- function(srate, notification = TRUE){
      is_set <- FALSE
    ...
    ## Voltage
    collect_raw_voltage <- function(force = FALSE){
      if(!utils$has_subject()){
        return(FALSE)
      }
    ...
    load_voltage <- function(electrodes, blocks, raw = FALSE, time = NULL){
      if(!utils$has_raw_cache()){
        return(NULL)
      }
      if(missing(blocks)){
    ...
    load_wavelet <- function(electrode, blocks, referenced = FALSE){
      ...
      sapply(blocks, function(b){
        power <- load_h5(fpower, name = sprintf('%s/power/%s', prefix, b), ram = TRUE)
        phase <- load_h5(fphase, name = sprintf('%s/phase/%s', prefix, b), ram = TRUE)
    ...
    apply_wavelet <- function(electrodes, target_srate, frequencies, wave_num, ncores = 4){
      ...
      srate <- utils$get_srate()
      dirs <- utils$get_from_subject('dirs', NULL, customized = FALSE)
      if(!length(dirs)){
      ...
      for(block in blocks){
        for(nm in c('raw', 'ref')){
          dir.create(file.path(dirs$channel_dir, 'cache', 'power', nm, block), recursive = TRUE, showWarnings = FALSE)
          dir.create(file.path(dirs$channel_dir, 'cache', 'phase', nm, block), recursive = TRUE, showWarnings = FALSE)
          dir.create(file.path(dirs$channel_dir, 'cache', 'voltage', nm, block), recursive = TRUE, showWarnings = FALSE)
        }
      }
      ...
      wavelet_log <- env$subject$logger$get_or_save('wavelet_log', list(), save = FALSE)
      ...
      safe_write_csv(data.frame(
        ...
        Reference = 'noref'
      ), file.path(dirs$channel_dir, 'cache', 'cached_reference.csv'), row.names = FALSE)
