...
#' @import stringr
#' @export
load_modules <- function(recompile = F){
  ...
  if(length(pkg_ids)){
    ...
    for(pkg_id in pkg_ids){
      ...
      if(recompile){
        ...
        compile = T
      }else if(!pkg %in% installed.packages()[,1]){
        logger('Package - [' %&% pkg %&% '] is not installed. Try to compile it.', level = 'INFO')
        compile = T
      }

      if(compile){
        suppressMessages(suppressWarnings(install_rave_modules(pkg_id)))
      }
...
create_package <- function(
  ...
  body(function_env$..get_runtime_env) = rlang::quo_squash(rlang::quo({
    ...
    if(is.null(..module_env[[module_id]])){
      ..module_env[[module_id]] = rave::ModuleEnvir$new(
        module_id = module_id,
